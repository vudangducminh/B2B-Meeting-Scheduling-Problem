...
for input_file in input_files:
    ...
    output_file = f'./incsat_output/{base_name}'
    ...
    # Monotonicity for max and min: max[j+1] -> max[j] and not min[j] -> not min[j+1]
    ...
    # Apply Itotalizer to sortedHole
    arr = []
    for p in range(1, nBusiness + 1):
        for t in range(1, max_break_count + 1):
            arr.append(sortedHole[p][t])
    ...
    best = nBusiness * max_break_count
    if not solver.solve():
        ...
        for p in range(1, nBusiness + 1):
            for t in range(1, max_break_count + 1):
                if sortedHole[p][t] in model_set:
                    max_bound += 1
        best = max_bound
        ...
        while r >= l:
            ...
            if solver.solve(assumptions=assumps):
                ...
                for p in range(1, nBusiness + 1):
                    for t in range(1, max_break_count + 1):
                        if sortedHole[p][t] in model_set:
                            cost += 1
                print(f"Found solution with cost {cost} (bound {mid})")
                if best > cost:
                    best = cost
                    assignment = model_set
                r = min(cost - 1, mid - 1)
            ...
            if time.time() - solve_start > 3600:
                print("Timeout reached during incremental search")
                break
    solver.delete()
    ...
    print(f"Incremental SAT solving completed in {solve_time - solve_start:.4f} seconds")
    ...
    with open(out_path, 'w') as f:
        ...
        f.write(f"Incremental SAT solving: {solve_time - solve_start:.4f} seconds\n")
        f.write(f"{'='*60}\n\n")
        f.write("Total variables: {}\n".format(variable_size))
        f.write("Total clauses: {}\n".format(len(cnf.clauses)))
        f.write("Incremental SAT found with cost: {}\n".format(best if best is not None else "N/A"))
        if assignment:
            # Checker
            f.write("SCHEDULE:\n")
            for m in range(1, nMeetings + 1):
